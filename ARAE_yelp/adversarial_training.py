...
def save_model():
    print("Saving models")
    with open('{}/autoencoder_model.pt'.format(args.modelf), 'wb') as f:
        torch.save(autoencoder.state_dict(), f)
    with open('{}/gan_gen_model.pt'.format(args.modelf), 'wb') as f:
        torch.save(gan_gen.state_dict(), f)
    with open('{}/gan_disc_model.pt'.format(args.modelf), 'wb') as f:
        torch.save(gan_disc.state_dict(), f)
    with open('{}/classifier_model.pt'.format(args.modelf), 'wb') as f:
        torch.save(classifier.state_dict(), f)
               
def load_model():
    print("Loading models from {}".format(args.modelf))
    if os.path.exists('{}/autoencoder_model.pt'.format(args.modelf)):
        autoencoder.load_state_dict(torch.load('{}/autoencoder_model.pt'.format(args.modelf)))
    if os.path.exists('{}/gan_gen_model.pt'.format(args.modelf)):    
        gan_gen.load_state_dict(torch.load('{}/gan_gen_model.pt'.format(args.modelf)))
    if os.path.exists('{}/gan_disc_model.pt'.format(args.modelf)):    
        gan_disc.load_state_dict(torch.load('{}/gan_disc_model.pt'.format(args.modelf)))
...
def train_gan_g():
    ...
    fake_hidden = gan_gen(noise)
    errG = gan_disc(fake_hidden).view(1)  # DEBUGGING: added view
    errG.backward(one)
...
def train_gan_d(whichdecoder, batch):
    ...
    # loss / backprop
    errD_real = gan_disc(real_hidden).view(1)  # DEBUGGING: added view
    ...
    # loss / backprop
    fake_hidden = gan_gen(noise)
    errD_fake = gan_disc(fake_hidden.detach()).view(1)  # DEBUGGING: added view
...
#load pre-trained model if required
...
for epoch in range(1, args.epochs+1):
    ...
    print('Classifier on Test 1, Loss:{} Accuracy:{}'.format(test_loss_classifier, accuracy_classifier))
